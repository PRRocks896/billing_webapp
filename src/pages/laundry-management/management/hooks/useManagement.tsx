...
import { deleteLaundaryManagement, getLaundryManagementList, updateLaundaryManagement } from "service/laundry-management";
...
const UseManagement = () => {
    ...
    const handleAdd = () => navigate("/laundry-management/laundry-management/add");

    const handleEdit = (id: number) => navigate(`/laundry-management/laundry-management/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getLaundryManagementList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteLaundaryManagement(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const Column: HeadCell[] = useMemo(() => {
        return [
            ...
            },
            {
                ...
                isSortable: true,
                renderCell: (row: any) => {
                    return (row?.px_laundry_washer?.name)
                }
            },
            {
                id: 'price',
