...
const UseHomePage = () => {
    ...
    const handleAdd = () => navigate("/website-management/home-page/add");

    const handleEdit = (id: number) => navigate(`/website-management/home-page/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getHomePageList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateHomePage(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteHomePage(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
