...
const UseWebSetting = () => {
    ...
    const handleAdd = () => navigate("/website-management/web-setting/add");

    const handleEdit = (id: number) => navigate(`/website-management/web-setting/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getWebSettingList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateWebSetting(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteWebSetting(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
