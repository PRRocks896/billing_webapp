...
import { deleteEmpWellnessPlan, getEmpWellnessPlanList, updateEmpWellnessPlan } from "service/empWellnessPlan";
import { HeadCell, ArrangementOrder } from "types/table";
import { ROWS } from "utils/constant";

const UseEmployeeWellnessPlan = () => {
    ...
    const handleAdd = () => navigate("/employee-wellness-plan/add");

    const handleEdit = (id: number) => navigate(`/employee-wellness-plan/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            const { success, message, data }: any = await getEmpWellnessPlanList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, data: any) => {
        try {
            ...
            const { success, message }: any = await updateEmpWellnessPlan(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            const { success, message }: any = await deleteEmpWellnessPlan(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
...
export default UseEmployeeWellnessPlan