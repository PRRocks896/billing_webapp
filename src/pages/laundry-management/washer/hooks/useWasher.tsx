...
import { deleteLaundryWasher, getLaundryWasherList, updateLaundryWasher } from "service/laundry-washer";
...
const UseWasher = () => {
    ...
    const handleAdd = () => navigate("/laundry-management/laundry-washer/add");

    const handleEdit = (id: number) => navigate(`/laundry-management/laundry-washer/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getLaundryWasherList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateLaundryWasher(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteLaundryWasher(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
