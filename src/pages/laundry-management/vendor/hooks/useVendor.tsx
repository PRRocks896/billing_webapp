...
import { deleteLaundryVendor, getLaundryVendorList, updateLaundryVendor } from "service/laundry-vendor";
...
const UseVendor = () => {
    ...
    const handleAdd = () => navigate("/laundry-management/laundry-vendor/add");

    const handleEdit = (id: number) => navigate(`/laundry-management/laundry-vendor/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getLaundryVendorList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateLaundryVendor(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteLaundryVendor(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
