...
import { deletePaymentBank, getPaymentBankList, updatePaymentBank } from "service/paymentBank";
...
const UsePaymentBank = () => {
    ...
    const handleAdd = () => navigate("/payment-bank/add");

    const handleEdit = (id: number) => navigate(`/payment-bank/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getPaymentBankList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updatePaymentBank(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deletePaymentBank(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
