...
import { deleteLaundryItem, getLaundryItemList, updateLaundryItem } from "service/laundry-item";
...
const UseItem = () => {
    ...
    const handleAdd = () => navigate("/laundry-management/laundry-item/add");

    const handleEdit = (id: number) => navigate(`/laundry-management/laundry-item/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getLaundryItemList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateLaundryItem(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteLaundryItem(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
