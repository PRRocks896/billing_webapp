...
const UseSeo = () => {
    ...
    const handleAdd = () => navigate("/website-management/seo/add");

    const handleEdit = (id: number) => navigate(`/website-management/seo/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getSeoList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateSeo(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteSeo(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
