...
const UseCity = () => {
    ...
    const handleAdd = () => navigate("/city/add");

    const handleEdit = (id: number) => navigate(`/city/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getCityList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateCity(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteCity(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
