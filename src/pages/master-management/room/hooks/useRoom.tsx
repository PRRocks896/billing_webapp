...
import { deleteRoom, getRoomList, updateRoom } from "service/room";
...
const UseRoom = () => {
    ...
    const handleAdd = () => navigate("/room/add");

    const handleEdit = (id: number) => navigate(`/room/edit/${id}`);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateRoom(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            const { success, message }: any = await deleteRoom(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
