...
import { deleteMembershipPlan, getMembershipPlanList, updateMembershipPlan } from "service/membershipPlan";
...
const UseMembershipPlan = () => {
    ...
    const handleAdd = () => navigate("/membership-plan/add");

    const handleEdit = (id: number) => navigate(`/membership-plan/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            const { success, message, data }: any = await getMembershipPlanList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, data: any) => {
        try {
            ...
            const { success, message }: any = await updateMembershipPlan(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            const { success, message }: any = await deleteMembershipPlan(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
