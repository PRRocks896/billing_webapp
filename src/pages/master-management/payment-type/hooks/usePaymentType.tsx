...
import { deletePaymentType, getPaymentTypeList, updatePaymentType } from "service/payment-type";
...
const UsePaymentType = () => {
    ...
    const handleAdd = () => navigate("/payment-type/add");

    const handleEdit = (id: number) => navigate(`/payment-type/edit/${id}`);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updatePaymentType(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deletePaymentType(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
