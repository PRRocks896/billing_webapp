...
const UseBlog = () => {
    ...
    const handleAdd = () => navigate("/website-management/blog/add");

    const handleEdit = (id: number) => navigate(`/website-management/blog/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getBlogList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateBlog(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteBlog(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
