...
import { deleteCompany, getCompanyList, updateCompany } from "service/company";
...
const UseCompany = () => {
    ...
    const handleAdd = () => navigate("/company/add");

    const handleEdit = (id: number) => navigate(`/company/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            const { success, message, data }: any = await getCompanyList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateCompany(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            const { success, message }: any = await deleteCompany(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
