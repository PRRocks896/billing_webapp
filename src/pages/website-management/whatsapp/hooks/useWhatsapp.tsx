...
const UseWhatsapp = () => {
    ...
    const handleAdd = () => navigate("/website-management/whatsapp/add");

    const handleEdit = (id: number) => navigate(`/website-management/whatsapp/edit/${id}`);
    ...
    const fetch = useCallback(async () => {
        ...
        try {
            startLoading();
            const { success, message, data }: any = await getWhatsappList(payload);
            if (success) {
                setList(data.rows);
    ...
    const onStatusChange = async (id: number, newStatus: boolean) => {
        try {
            ...
            const { success, message }: any = await updateWhatsapp(body, id);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
    ...
    const onDeleteHandler = async () => {
        try {
            startLoading();
            const { success, message }: any = await deleteWhatsapp(selectedId);
            if (success) {
                fetch();
                openSnackbar({
                    open: true,
