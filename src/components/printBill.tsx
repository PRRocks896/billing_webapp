  ...
  return `<html>
    ...
    <body>
      <div style="padding: 0mm; margin: 0 auto; width: 80mm;">
        ${billData.tableData?.map((bill, idx, arr) => (`
          <div style="${idx < arr.length - 1 ? 'page-break-after: always;' : ''} border: 0px solid black; min-height: max-content;">
            <p style="text-transform: capitalize;font-size: 14px; font-weight: 600; margin: 0px;text-align: center; margin-bottom: 0px">${branchData.title
    }</p>
      ...
      : ''}
            <div style="display: flex; justify-content: space-between;margin-top: 7px;">
              <div>
                ...
                <p style="text-align: start; margin: 0px; font-size: 14px;">Bill No: ${bill.billNo
    }</p>
              </div>
              <div>
            ...
            <table style="width: 100%;">
              ...
              <tbody>
                <tr style="text-align:center;font-size: 14px;">
                  ...
                  <td>${bill?.hsnCode || ''}</td>
                  <td>${bill.quantity}</td>
                  <td>${showTwoDecimal(bill.total as number)}</td>
                  <td>${showTwoDecimal(bill.total as number)}</td>
                </tr>
              </tbody>
            </table>
            <div style="width: 100%; border-top: 1px dashed black;border-bottom: 1px dashed black; display: flex; justify-content: end;font-size: 14px;">
              ...
              <p style="margin: 5px 0px; margin-right: 4px; font-weight: 600; text-align: end;">${showTwoDecimal(bill.subTotal as number)
    }</p>
            </div>
            ${billData.isShowGst ?
      `<div style="width: 100%; border-top: 1px dashed black;border-bottom: 1px dashed black; display: flex; justify-content: end;font-size: 14px;">
      ...
      `<p style="margin: 5px 0px; margin-right: 10px; font-weight: 600;">CGST (${billData.cgstPercentage}%): </p>
                  <p style="margin: 5px 0px; margin-right: 4px; font-weight: 600; text-align: end;">${showTwoDecimal(bill.cgst as number)
      },</p>`
      }
                ${bill.sgst &&
      `<p style="margin: 5px 0px; margin-right: 10px; font-weight: 600;">SGST (${billData.sgstPercentage}%): </p>
                  <p style="margin: 5px 0px; margin-right: 4px; font-weight: 600; text-align: end;">${showTwoDecimal(bill.sgst as number)
      }</p>`
      }
              </div>`
      : ''}
            <div style="width: 100%;border-bottom: 1px dashed black; display: flex; justify-content: end;font-size: 18px;">
              ...
              <p style="margin: 5px 0px; margin-right: 4px; font-weight: 600; text-align: end;">${showTwoDecimal(bill.grandTotal)
    }</p>
            </div>
            <div style="width: 100%;border-bottom: 1px dashed black; display: flex; justify-content: start;font-size: 10px;">
              ...
              <p style="margin: 5px 0px; margin-left: 10px;">${convertAmountToWords(bill.grandTotal).toUpperCase()
    } RUPEES</p>
            </div>
      ...
      : ""
              ...
              <div style="display: flex;">
                ...
                <p style="margin: 5px 0px; margin-left: 10px;">${showTwoDecimal(bill.grandTotal)
    }</p>
              </div>
            </div>
            <div style="width: 100%;border-bottom: 1px dashed black; display: flex; justify-content: start; flex-direction: column; font-size: 14px;">
